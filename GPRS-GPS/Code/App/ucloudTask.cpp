...
//			OSTaskSuspend(APP_TASK_SERVER_RX_PRIO);
		...
		switch(ServerPackt.msgHead.msgid)
		{
			...
			case JCLOUD_MSG_ID_SETTING_REQ:
			{
				...
				bool back_sw = _setting&0x01;                //返航指令
				bool parachute_sw = _setting>>1&0x01;        //开伞指令
				bool buzzer_sw = _setting>>2&0x01;           //蜂鸣器指令
				bool buckUpGps_sw = _setting>>3&0x01;           //蜂鸣器指令
				
				if(buzzer_sw) {BEEP_ON;}   //蜂鸣器判断
				else {BEEP_OFF;}
